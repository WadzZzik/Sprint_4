...
public class MainPageAccordionTests {
    ...
    //Порядковый номер элемента дропдаунов
    private final int numberOfElement;

    //Ожидаемый текст в заголовке элемента дропдаунов
    private final String expectedHeaderText;

    // Ожидаемый текст в раскрывающемся блоке элемента дропдаунов
    ...
    public static Object[][] setTestData() {
        return new Object[][] {
                ...
                {7, "Я живу за МКАДом, привезёте?", "Да, обязательно. Всем самокатов! И Москве, и Московской области." }, // Дефект тут -> в
        };
    }
    ...
    //Тест для проверки работы дропдауна и для проверки текста в заголовках и в раскрывающихся блоках
    ...
    public void checkAccordionIsCorrect() {
        ...
        if (mainPage.isAccordionItemDisplayed(this.numberOfElement)) {
            MatcherAssert.assertThat("Проблема с текстом в заголовке дропдауна" + this.numberOfElement,
            ...
            fail("Заголовок дропдауна" + this.numberOfElement + "не загрузился");
        }
    }
}
